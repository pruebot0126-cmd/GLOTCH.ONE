...
const drawControl = new L.Control.Draw({
  draw: { polygon: true, rectangle: false, polyline: false, circle: false, marker: false },
...
map.on("draw:edited", function(e) {
  e.layers.eachLayer(layer => updateArea(layer.toGeoJSON()));
});
...
function loadShapes() {
  ...
  savedShapes.forEach(shape => L.geoJSON(shape).addTo(drawnItems));
  alert("Formas cargadas.");
}

// Satélite
function toggleSatellite() {
  if (satelliteMode) { map.removeLayer(satelliteLayer); map.addLayer(streetLayer); }
  else { map.removeLayer(streetLayer); map.addLayer(satelliteLayer); }
